...
*/
mugs.Multimap = function(keyValuePairs, collectionConstructor, comparator) {
  var kv, treeUnderConstruction, val, _i, _len;
  ...
  if (keyValuePairs instanceof Array && keyValuePairs.length > 0) {
    for (_i = 0, _len = keyValuePairs.length; _i < _len; _i++) {
      kv = keyValuePairs[_i];
      val = (kv.value instanceof Array ? new collectionConstructor(kv.value) : new collectionConstructor([kv.value]));
  ...
  if (comparator != null) {
    this.tree_.comparator = comparator;
  }
  return this;
};
...
*/
mugs.Multimap.prototype.insert = function(key, value) {
  var current_list, new_tree;
  ...
  return this.buildFromTree(new_tree);
};
/*
...
*/
mugs.Multimap.prototype.get = function(key) {
  return this.tree_.get(key).getOrElse(new this.collectionConstructor_());
};
/*
...
*/
mugs.Multimap.prototype.remove = function(key, value) {
  var currentList, newList, new_tree;
...
*/
mugs.Multimap.prototype.removeAll = function(key) {
  return this.buildFromTree(this.tree_.remove(key));
};
/*
...
*/
mugs.Multimap.prototype.contains = function(key) {
  return this.tree_.containsKey(key);
};
/*
  ...
  @returns a mugs.List with all of the values in the Multimap
*/
mugs.Multimap.prototype.values = function() {
  return this.tree_.values().flatMap(function(list) {
    return list;
  });
};
/*
  ...
  @returns a mugs.List with all of the keys in the Multimap
*/
mugs.Multimap.prototype.keys = function() {
  return this.tree_.keys();
};
/*
  @private
*/
mugs.Multimap.prototype.buildFromTree = function(tree) {
  var map;
