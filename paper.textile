...
When methods return null you always have to check if a value is null before using it in subsequent expressions. This will clutter the programs with a lot of @if( value != null ) { }@ statements which hide the true intent of the program. When using Option you can chain operations without having to check for the existence of a value before the very last expression where the developer can supply a fallback value using the @getOrElse()@ method. This is done using the @map@, @flatMap@, @filter@ and @foreach@ methods. This results in code that is shorter and more concise as the above example show. It is possible to chain operations on Option because that all operations on @None@ return @None@ except for @getOrElse(value)@ which return the value. @getOrElse(value)@ on @Some@ will return the element stored in the container and ignore @value@.
...
<table>
	...
	<tr>
		...
		<td>Tests if the set contains the element</td>
		<td>@O(log n)@</td>
	</tr>
</table>
...
A stack inserts and retrieves elements according to the last-in-first-out (LIFO) principle. The Stack Abstract Data Type has been implemented using a List because of its quick access, insertion and removal of the first element. 
...
A queue inserts and retrieves elements according to the first-in-first-out (FIFO) principle. The Queue Abstract Data Type has been implemented using two Lists.
...
<pre><code>var PersonFunctional = function(firstName, lastName){
...
new PersonFunctional("Mads","Hartmann");</code></pre>

The advantage of this approach is that you can define any variables you want inside of the function and they will act as private members. The disadvantage is that instances created with this function will be heavy objects in the sense that all of them will store the code (TODO: Can't figure out if it stores the entire AST on each object) for each method. 
...
<pre><code>var PersonClassical = function(firstName, lastName){
	this.firstName = firstName;
	this.lastName  = lastName;
	return this; 
};
...
new PersonClassical("Mads","Hartmann");</code></pre>

This resolves the disadvantage of the Functional approach as all the instances share the same prototype object. This means that object creation is faster because the interpreter doesn't have to add the same methods to all of the objects. This also results in objects that use less memory.
...
<pre><code>var PersonPrototypal = {
	fullName: function() { return this.firstName + " " + this.lastName; }
};
Object.create(PersonPrototypal, {
	firstName: { 
		value: "Mads"
	}, 
	lastName: { 
...
This approach allows for great flexibility and shows JavaScripts true prototypal nature. Unfortunately Object.create isn't implemented in all modern browsers.

h4. Comparisons

The following table shows the results of testing the performance of the three different approaches. The tests show the number of operations per second the javascript machine can execute when creating a new object based on each approach. The test framework JSLitmus has been used to execute the tests. 
...
<table>
	...
	<tr>
		...
		<td>2301285 Ops/sec</td>
		<td>14842260 Ops/sec</td>
		<td>758273 Ops/sec</td>
	</tr>
	<tr>
		<td>Chrome (Version 10.0.648.127 on OS X)</td>
		<td>26421811 Ops/sec</td>
		<td>45519014 Ops/sec</td>
		<td>171517 Ops/sec</td>
	</tr>
	<tr>
		<td>Firefox (Version 3.6.13 on OS X)</td>
		<td>1485148 Ops/sec</td>
		<td>1969373 Ops/sec</td>
		<td>Not supported</td>
	</tr>
</table>

The table shows that the Pseudoclassical approach is the most performant solution across all the testes browsers and as such this is the approach I'm using in the library.

The following code shows the JSLitmus tests used to produce the output above. The tests use the functions and objects shown earlier in this section.

<pre>JSLitmus.test('Creating object - Functional', function() {
	new PersonFunctional("Mads","Hartmann");
});

JSLitmus.test('Creating object - Classical', function() {
	new PersonClassical("Mads","Hartmann");
});

JSLitmus.test('Creating objects - Prototypal',function() {
	Object.create(PersonPrototypal, {
		firstName: { 
			value: "Mads" 
		}, 
