...
This section explains how the library deals with methods where a successful result isn't guaranteed. An example of this could be the @get(key)@ method on Map. In cases like this the convention is to return an instance of Option instead of null or throwing an exception. Instances of Option are either an of the type of Some or None. Some is a single element container which contains the result of successfully executing the method. The use of the Option type is adopted from the Scala standard library. Readers coming from a functional programming background might recognize Option as a Monad.  

To display the power of Option I've created two applications that will get a value from one Map and use that value to receive a second value from another map and finally print that value to the console. The first solutions takes an imperative approach to the problem and assumes that the get() method on Map will return null if the key doesn't exist. This is similar to the Map class in the standard Java collection. The second solution takes a functional approach and the @get()@ method on Map returns an Option. 

Both solutions assume that the following two maps are declared.

<pre><code>persons = new Map({ key: 'Mads', value: 1234 })
details = new Map({ key: 1234, value: { address: 'Grundtvigs Sidevej 1 st th' } })</code></pre>

The following is the imperative solution:
...
There are two things to take away from these examples. The first is that the most idiomatic way to deal with Option is to treat it as a collection and use the <code>map</code>, <code>flatMap</code>, <code>filter</code> and <code>foreach</code> methods. This allows the develop to chain operations on Option without having to check for the existence of a value before the very last expression where the developer can supply a fallback value using the @getOrElse()@ method. This results in code that's shorter and more concise as the above examples show. It is possible to chain operations on Option due to the fact that all operations on @None@ return @None@ except for @getOrElse(value)@ which return the value. @getOrElse(value)@ on @Some@ will return the element stored in the container and ignore @value@.

The second advantage that @Option@ has over null/exceptions is that it *always* forces the developer to consider the possibility of an error as it isn't possible to access the values directly. This is compared to null/exceptions where the developer would have to read the source code or documentation to realize that an error might occur. 
...
There are many different ways to create and initialize a new object in javascript and each one comes with its own set of merits and penalties. In this section I will introduce 3 different ways to construct objects in javascript and explain which one I've chosen for this library and why. 

h4(#factory_functions). Factory Function

In the Factory Function approach you create a function which acts as the constructor for the object you want to create. It can take any number of arguments and it uses those arguments to add properties and methods to the object. The example below shows a Person factory function which takes two arguments: firstName, lastName. It uses those arguments to create two properties and a method on the object.
...
h4(#prototype). Classical  

In the Classical approach you create a function which only initializes the properties of the object. Instead of adding the methods directly on each object you add it to the prototype as shown below. 
...
h4(#object_create). Object.create
...
This approach allows for great flexibility and shows JavaScripts true prototypal nature. Unfortunately Object.create isn't implemented in all modern browsers and in the ones where it is implemented the Object.create application isn't as performant as the Classical one. Because of this I've chosen to use the Classical approach in this library. 

h3(#javascript_call_stack). Javascript call-stack

Most JavaScript implementations have a fixed upper bound on the size of the call-stack. The following table shows the maximum size of the call-stack in different browsers. 

<table>
	<thead>
	<tr>
		<td> Browser </td>
		<td> Max size of call-stack</td>
	</tr>
	</thead>
	<tr> 
...
This, unfortunately, means that recursive functions have the possibility of breaking the application. Seeing that recursive functions are one of the backbones of  functional programming this is a huge restriction on how elegantly I've been able to implement the different data structures. For methods like map which are applied on every element in a collection I've been forced to rely on side-effects as a recursive function would break the application when applied on collections with more than 3000 (in the case of Firefox) elements. 
