...
In the Factory Function approach you create a function which acts as the constructor for the object you want to create. It can take any number of arguments and it uses those arguments to add properties and methods to object. The example below shows a Person factory function which takes two arguments: firstName, lastName. It uses those arguments to create two properties and a function on the object.
...
In the Classical approach you create a function which only initializes the properties of the object. Instead of adding the methods directly on each object you add it to the prototype instead as shown below. 
...
There are two disadvantages of this approach. Firstly, because the methods are no longer stored directly on the object the interpreter needs to do, at least, one extra lookup before it can execute the method. Because of this it takes roughly %40 longer to invoke a method on an object. The second disadvantage is that it's no longer possible to the objects to have private state. 
...
Object.create(Person, {firstName: "Mads", lastName: "Hartmann"});</code></pre>
